  ...
  ,m_processNum(0)
{
    ...
    ci_autoCrop = new CImageApplyAutoCrop();
...
void procthread::setResizeScale(int scale_x, int scale_y)//按尺寸
{
    ci_resize->setDSize(cv::Size(scale_x,scale_y));
}
...
void procthread::run()
{
    if(m_isRunning)
    {
        for (const QString& filename : m_filenames)
        {
            ...
            ci_autoCrop->setFixedSize(cv::Size(image.cols,image.rows));//获取裁剪原始尺寸
